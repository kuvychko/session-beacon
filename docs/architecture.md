...
Device discovery: `--port COMx` explicit, else auto-detect by USB VID/PID via pyserial's port listing, matching either board: the Nano ESP32 (0x2341:0x0070) or an RP2040 (0x2E8A:0x0003, which is the generic RP2040 pair rather than the Zero's own). This is the only board-specific part of the host. The protocol and everything behind it are identical for both builds, and the heartbeat's `board` field names the one that answered.
...
- Sends a heartbeat every 3 s. It is the host's only proof that the sketch is still running; see [the freeze below](#usb-writes-must-never-wait).
- Accepts local commands on the same serial line so the layout can be exercised without a host. A line starting with `{` is a protocol message; anything else is a command. `demo` loads a canned snapshot with running timers, and `hang` stops `loop()` on purpose to check that the watchdog reboots the board.
- Reports counters in its heartbeat: lines received, parse failures, dropped lines, lines it could not send, milliseconds since the last accepted message, the last render duration, and why the board last reset. These exist because a quiet host and a device that is dropping or failing to parse lines look identical from the outside, namely a screen reading "no host".
- **Never writes to `Serial` directly.** Output goes through `txLine()` into a queue, and `txPump()` hands the USB stack only as many bytes as it has room for.
- **The loop watchdog is on** (`enableLoopWDT()`, 5 s). A hung `loop()` reboots the board instead of freezing it.

### USB writes must never wait

The display froze on hours-old rows eight times in eight days. Only a power cycle fixed it. The panel looked healthy, but nothing on it moved: no footer page flip, no pulsing, no "no host". All three are driven from `loop()`, so `loop()` had stopped. The board stayed enumerated, Windows accepted every write, and `/health` said `"device": true`, because the host only checked that its port handle was open. One freeze lasted 7 h 26 min.

The cause is in the core, and it is the Nano's. On the Nano ESP32, `Serial` is TinyUSB's `USBCDC`. Its `write()` loops until every byte fits in a 64-byte FIFO, and that loop has no timeout: `setTxTimeoutMs()` bounds only the lock. When the PC stopped collecting from the port while DTR stayed asserted, the next heartbeat's `Serial.printf` spun forever. USB selective suspend is the likely trigger, since each freeze began with a single `WriteFile failed` on the host. Nothing rescued it, because the loop task on core 1 is not watched by the task watchdog by default, and the ST7735 keeps its last frame with no help from the MCU.

Three changes, all worth keeping:
...
- The RP2040 core is not exposed to the original fault: its `SerialUSB::write()` gives up after a second. Both builds queue anyway, because a second is still most of a frame and the queue costs nothing.
...
What the firmware *can* do is notice. Receiving snapshots while not one byte leaves the board is a flat contradiction, and it is the only evidence of this state anywhere on either side of the link. `txWatchdog()` holds it for 30 s -- long enough that a genuine unplug resolves itself, since with no host there is no inbound traffic either -- and then does what users were doing by hand: re-enumerates the device by rebooting it. The host already treats a reboot as an ordinary reconnect.

The first version watched `hostAttached()` instead of bytes, and missed the next episode entirely. The board rendered every snapshot for over five hours, across five daemon restarts, without sending one heartbeat and without once rebooting; a raw read of the port with the daemon stopped got zero bytes in 25 s. Either the flag read *true* the whole time over a FIFO that never drained -- `availableForWrite()` stuck at 0, lines queuing and then dropping -- or it flickered back to true more often than every 30 s, and each flicker restarted the count. Both leave the flag-based check blind, and bytes written covers both as well as the case it was built for, because a board with a host is never quiet: it queues a heartbeat every 3 s. So the timer runs while snapshots are arriving and nothing has been written, and any byte that leaves resets it.

The cure is invisible by the time anyone looks, so it is counted in memory that survives the software reset -- RTC memory on the ESP32, the watchdog's scratch registers on the RP2040, which has none -- and reported in every later heartbeat as `wedge`. A rise in it is logged by the host. The same memory keeps what the episode looked like, because the report above could not say which of the two it was: `wcause` is what the flag read at the moment of the reboot, and `wflip` how often it changed during the episode, so the next occurrence names itself -- a steady `det` is a stale flag, `att` a stopped FIFO, and a high `wflip` a link flapping under selective suspend. `aflip` counts how often the flag has changed at all since boot, so a link that is flapping but recovering can be seen before it sticks. The `wedge` serial command forces the flag false and `stall` stops the writes with the flag true, to test both paths on the bench as `hang` does for the loop watchdog.
