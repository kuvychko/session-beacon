...
The cause is in the core. On the Nano ESP32, `Serial` is TinyUSB's `USBCDC`. Its `write()` loops until every byte fits in a 64-byte FIFO, and that loop has no timeout: `setTxTimeoutMs()` bounds only the lock. When the PC stopped collecting from the port while DTR stayed asserted, the next heartbeat's `Serial.printf` spun forever. USB selective suspend is the likely trigger, since each freeze began with a single `WriteFile failed` on the host. Nothing rescued it, because the loop task on core 1 is not watched by the task watchdog by default, and the ST7735 keeps its last frame with no help from the MCU.
